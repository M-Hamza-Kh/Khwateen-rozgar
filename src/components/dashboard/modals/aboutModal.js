...
const AboutModal = (props) => {
    const {openAboutModal, defaultVal,onSave,onClose} = props;
    const [dVal, setDVal] = useState(defaultVal);

    const handleUpdate = () => {
        let aboutObj = {
            type:STRINGS.TYPES.PROFILE_UPD.ABOUT,
            data:dVal
        }
        onSave(aboutObj);
        onClose()
    ...
    return (
        <React.Fragment>
            ...
            <div
                className={`modal fade profile-modal-container ${openAboutModal ? `show` : ``}`}
                id="about-modal"
                        ...
                        <div className="modal-body">
                            <section className="card">
                                <header className="card-header">
                                    <h2 className="card-title mb-0">Edit
                                        About</h2>
                                </header>
